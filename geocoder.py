...
class Geocoder(object):
    ...
    """
    def __init__(self,dataset="geolite"):
        """
        ...
        if dataset == "geolite" or dataset == "geo-median":
            ...
            for cols in data[1:]:
                ...
                city = cols[2].lower()
                country = cols[3].lower()
                states = cols[4].lower().split('|')
            ...
            for line in data[1:]:
                ...
                country_name = line[1].lower()
                region_name = None#line[1].lower()
...
def geolite_data():
    ...
    """
    file_contents = []
    file_name = os.path.join("resources","geolite.csv")
...
def geonames_data():
    ...
    """
    file_contents = []
    #file_name = os.path.join("resources","geonames.exanded-cities.tsv.gz")
    file_name = 'city_collapsed_all.txt'
    #f = gzip.open(file_name, 'rb')
    f = open(file_name, 'r') 
    for line in f:
        line = line.strip().split('\t')
        line = [x for x in line if x!='']
        cityName = line[0].split('-')
        line = [cityName[0]]+[cityName[2]]+line[1:]
#        print line
        file_contents.append(line)
        file_contents.append([line[0]]+['']+line[2:])
    f.close()
    return file_contents
...
def dbpedia_data():
    ...
    Returns the file contents of the google dataset.
    """
    #file_name = os.path.join("resources","dbpedia.settlement-to-locations.localized.cleaned.uniq.tsv")
    file_name = os.path.join("resources","dbpedia-placenames.tsv")
    file_contents = []
    with open(file_name, 'r') as tsv_file:
        for line in csv.reader(tsv_file, dialect="excel-tab"):
            file_contents.append(line)
